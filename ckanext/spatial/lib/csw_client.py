...
class CswService(OwsService):
    ...
    #def __init__(self, endpoint=None):
    #	log.info("CSW Init  %r", self )

    #     super(CswService, self).__init__(endpoint)
    #    log.info("CSW Init  %r", self )

         # self.sortby = SortBy([SortProperty('Identifier', 'ASC')])
         #log.info("CSW Init  %s", self.url )
    ...
    def getrecordbyid(self, ids=[], esn="full", outputschema="gmd", **kw):
        log.info('Making CSW request: getrecordbyid %r', ids)
        from owslib.csw import namespaces
        csw = self._ows(**kw)
        log.info('Making CSW request: getrecordbyid %r', ids)
