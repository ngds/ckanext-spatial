    ...
    for resource in package_dict['resources']:
        if "NGDS Tier 3 Data, csv format".lower() in resource['name'].lower():
            if resource['url'] in csv_file_arr.keys():                  
                ...
                try:
                    # log.info("before create context data dict %s", data_dict ) 
...
class SpatialHarvester(HarvesterBase):
    ...
    def get_package_dict(self, iso_values, harvest_object):
        ...
        if 'tags' in iso_values:
            for tag in iso_values['tags']:
                # changed this line from 50 to 99 - GH 8/27/15
                tag = tag[:99] if len(tag) > 99 else tag
		...
		log.info(">>>>>>>>>>>>>>>>>>>>>>> Resp Org 353 %s" % party )
		    	...
		    	parties[party['organisation-name']] = party['organisation-name']
			log.info("Resp Org Assigned 359 as %s" % party['organisation-name'] )
    ...
    def import_stage(self, harvest_object):
		...
		log.info('>>>>>>>>>>>>> spatial harvester base ISpatial 495 ') 
            ...
            log.info('>>>>>>>>>>>>> spatial harvester base - b4 Validate 509 ') 
	    ...
	    log.info('>>>>>>>>>>>>> spatial harvester base - pARSE iso DOC 522 ') 
        ...
        if iso_guid and harvest_object.guid != iso_guid:
            ...
            existing_object = model.Session.query(HarvestObject.id) \
                            ...
                            .first()
            if existing_object:
                self._save_object_error('Harvest GUID different than content GUID Object {0} already has this guid {1}'.format(existing_object.id, iso_guid),
                        harvest_object, 'Import')
                return False
        ...
        log.info('>>>>>>>>>>>>>> Got the package 579  - %s',package_dict['title'])

        for harvester in p.PluginImplementations(ISpatialHarvester):
            log.info('Harvester plugin 582')
                    ...
                    for resource in package_dict['resources']:
                        if "NGDS Tier 3 Data, csv format".lower() in resource['name'].lower():
                            ...
                            if len(key_arr) > 0:
                                ...
                                cmv_bits = key_arr[2].split('/')
                                cmv_bits.pop()                          
                                cm_model = "/".join(cmv_bits) + "/"
                                log.info('Content Model %s state: %s tag: %s layer: %s version: %s', cm_model, valid, usgin_tag, layer.layer_name, key_arr[2] )

                                if valid and messages:
                                    if dataCorrected:
                                        extras  = package_dict['extras']
                                    	for extra in extras: 
                                        	if extra['key'] == 'md_package':
                                           		md_pack =  json.loads(extra['value'])
                                                        md_pack['resourceDescription']['usginContentModel'] = cm_model
                                           		md_pack['resourceDescription']['usginContentModelLayer'] = layer.layer_name
                                           		md_pack['resourceDescription']['usginContentModelVersion'] = key_arr[2]
                                           		extra['value'] = json.dumps(md_pack)  
                                ...
                                if valid and not messages:
                                    ...
                                    csv_file_arr[resource['url']] = out
                                    log.info(' Valid and no messages resource URL %s  ', resource['url'])
        ...
        if status == 'new':
	    # log.info('>>>>>>>>>>>>>>> spatial.harvester.base New package 707 calls logic.Action default_create ')
            package_schema = logic.schema.default_create_package_schema()
            package_schema['tags'] = tag_schema
            context['schema'] = package_schema

	    # log.info('>>>>>>>>>>>>>>> spatial.harvester.base After New package 712 %s ' % package_schema )
            ...
            try:
                ...
                self._save_object_error('Validation Error 733 : %s' % str(e.error_summary), harvest_object, 'Import')
                return False
            ...
            if previous_object and previous_object.harvest_source_id <> harvest_object.harvest_source_id:
                log.info('harvest object {0} {1} changed harvest sources.format(harvest_object.id, harvest_object.package_id )')
